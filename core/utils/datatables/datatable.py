...
from .base import DTColumn
from .base import DTColumnOrder
from .base import DTDataCallbacks
from .base import DTParams
...
class DataTable:
    ...
    async def generate(self):
        try:
            await self.run(self.request_params)
    ...
    async def _get_records_total(self, session: Session) -> int:
        # total record count before filtering
        result: int | None = await session.scalar(select(func.count()).select_from(self.table))
        return 0 if result is None else result

    @staticmethod
    async def _get_records_filtered(session: Session, subquery: Subquery) -> int:
        result: int | None = await session.scalar(select(func.count()).select_from(subquery))
        return 0 if result is None else result
    ...
    def _add_order_criteria(self, stmt: Select[Any]) -> Select[Any]:
        ...
        for order in self.params.order:
            ...
            stmt = stmt.order_by(column.asc()) if order.is_asc else stmt.order_by(column.desc())
        return stmt
    ...
    async def _get_data(self, session: Session, stmt: Select[Any]) -> list[dict[str, Any]]:
    ...
    async def run(self, request_params: dict[str, Any]) -> None:
        self.params = self._parse_params(request_params)
        # get total record count from the table
        self.records_total = await self._get_records_total(self.engine)
        ...
        self.records_filtered = await self._get_records_filtered(self.engine, stmt.subquery())
        # get the filtered records from the database
        self.data = await self._get_data(self.engine, stmt)
