...
async def page_auth_profile(req: page_req):
    page.addContext(
        "data_user", await UserQueryService().get_user_by(username=req.user.username)
    )
...
async def page_auth_profile_save_ganti_password(
    dataIn: GantiPasswordRequest, req: page_req
):
    data_get = await UserQueryService().get_user_by(req.user.username)
    if data_get is None:
        raise UserNotFoundException

    if not await UserQueryService().verify_password(
        user=data_get, plain_password=dataIn.password_lama
    ):
            errors = [
                {
                    "loc": ["body", "password_lama"],
                    "msg": "password lama gagal dicocokkan",
    ...
    sleep(1)
    await UserCommandService().update_user_password(
        data_get.id, dataIn.password_baru1, dataIn.password_baru2
    )
